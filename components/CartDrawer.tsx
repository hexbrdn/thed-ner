...
export default function CartDrawer() {
  ...
  const validate = () => {
    ...
    if (form.name.trim().length < 2) e.name = "Bitte vollen Namen eingeben.";
    if (!/^[0-9\s()+-]{8,17}$/.test(form.phone.trim())) e.phone = "Gültige Telefonnummer erforderlich.";
    if (form.address.trim().length < 6) e.address = "Gültige Adresse erforderlich.";
    setErrors(e);
  ...
  const submit = () => {
    if (!validate()) return;
    const no = `HDD-${String(Date.now()).slice(-6)}`;
    setOrderNo(no);
    setStage("done");
    clear();
  };
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-line shrink-0">
          <div>
            ...
            <p className="font-display font-extrabold text-xl text-bone">
              {stage === "done" ? t.cart.orderReceived : `${count} ${t.cart.title}`}
            </p>
          </div>
          <button
            ref={closeRef}
              ...
              <div className="border-t border-line px-6 py-5 shrink-0">
                ...
                {delivery > 0 && (
                  <p className="text-xs text-smoke/70 mb-3">
                    {t.cart.freeDeliveryThreshold.replace("{amount}", String(FREE_OVER - total))}
                  </p>
                )}
                ...
                <button
                  ...
                  className="focus-ring w-full bg-flame-gradient text-void font-display font-extrabold py-3 hover:brightness-110 transition-[filter,transform] active:translate-y-px text-xs tracking-wider"
                >
                  {t.cart.addressBtn}
                </button>
              </div>
